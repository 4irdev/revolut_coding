...
from lib.transfer_service import Account, TransferService
import pytest
from time import time
import threading

@pytest.mark.parametrize("num_threads, transfer_amount", [(5, 50)])
def test_transfer_service_concurrency(num_threads, transfer_amount):
    acc1 = Account(1, 1000)
    acc2 = Account(2, 1000)
    initial_total = acc1.get_balance() + acc2.get_balance()

    def transfer():
        TransferService.transfer_money(acc1, acc2, transfer_amount)
