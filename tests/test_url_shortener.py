...
def test_random_strategy_collision():
    ...
    short_urls = [shortener.encode_url("https://collision-test.com") for _ in range(1000)]
...
def test_decode_url_correctness():
    shortener = URLShortener(CounterStrategy())
    mapping = {url: shortener.encode_url(url) for url in test_urls}

    for url, short in mapping.items():
        assert shortener.decode_url(short) == url
