...
from lib.transfer_service_queue import Account, QueueTransferService
from decimal import Decimal
from threading import Thread
from time import time


def test_transfer_service_queue_concurrency():
    acc1 = Account(1, Decimal(1000))
    acc2 = Account(2, Decimal(500))
    service = QueueTransferService()
    initial_total = acc1.balance + acc2.balance
    num_threads = 20
    amount = Decimal(50)

    def transfer():
        service.transfer_money(acc1, acc2, amount)
    ...
    service.transaction_queue.join()
